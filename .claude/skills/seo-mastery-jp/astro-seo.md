...
プラグインで解決している問題の多くを最初から解消しています。残るのはAstroが「開発者側の責任」と
して残している判断です。どのアイランドをハイドレートするか、ビルド時に絶対URLをどう組み立てるか、
...
OG画像は絶対ホストを含む絶対URLでなければなりません。`new URL(..., Astro.site)`ならそれが保証
...
Content Collectionsは全エントリにZodスキーマによる検証を与えます。このスキーマは構造化データが
必要とする契約そのものです。ここからJSON-LDを生成すれば、構造化データの不備はSearch Consoleの
エラーではなくビルドエラーになります。
...
2. **`<ClientRouter />`** — サイトをクライアントルーティング化し、`transition:persist`、遷移をまたぐ
   状態保持、ライフサイクルフックを可能にします。Astro公式ドキュメントもトレードオフを明記して
   います。「遷移後にスクリプトや状態を手動で再初期化する必要があるといった欠点がある」。
...
- [ ] JSON-LDはコレクションスキーマから生成し、`set:html`の前に`<`を`\\u003c`へエスケープしている
